...
function saveEdit(id) {
    editForm.put(route('equipment-categories.update', id), {
        onSuccess: () => { editingId.value = null; },
    });
}

function destroy() {
    router.delete(route('equipment-categories.destroy', deleteId.value), {
...
<template>
    <Head :title="t('equipment_categories')" />

    <AuthenticatedLayout>
        <template #header>
            <h1 class="text-xl font-bold text-slate-900 dark:text-slate-100">{{ t('equipment_categories') }}</h1>
        </template>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <tr v-for="cat in categories" :key="cat.id">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ cat.catalogs_count }}</span>
                            </td>
                            <td class="px-4 py-3 text-end">
                                ...
                                <div v-else class="flex justify-end gap-2">
                                    ...
                                    <button @click="deleteId = cat.id" :disabled="cat.catalogs_count > 0" :title="cat.catalogs_count > 0 ? t('category_in_use') : ''"
