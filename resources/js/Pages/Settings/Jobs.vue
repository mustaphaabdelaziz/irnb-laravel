...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="mx-auto max-w-2xl space-y-6">
            <!-- Add form -->
            <form @submit.prevent="addJob" class="flex items-end gap-3 rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
                <div class="flex-1">
                ...
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <tr v-for="job in jobs" :key="job.id">
                            <td class="px-4 py-3">
                                <TextInput v-if="editingId === job.id" v-model="editForm.name" class="w-full" />
                                <span v-else class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ job.name }}</span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-end">
                                ...
                                <div v-else class="flex justify-end gap-2">
                                    ...
                                    <button @click="deleteId = job.id" class="text-sm text-rose-500 hover:text-rose-700">{{ t('delete') }}</button>
                                </div>
                            </td>
                        </tr>
                        <tr v-if="!jobs?.length">
