...
function saveEdit(id) {
    editForm.put(route('positions.update', id), {
        onSuccess: () => { editingId.value = null; },
    });
}

function destroy() {
    router.delete(route('positions.destroy', deleteId.value), {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h1 class="text-xl font-bold text-slate-900 dark:text-slate-100">{{ t('positions') }}</h1>
        </template>

        <div class="mx-auto max-w-3xl space-y-6">
            <!-- Add form -->
            <form @submit.prevent="addPosition" class="flex items-end gap-3 rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
                <div class="w-24">
                    <label class="text-sm font-medium text-slate-700 dark:text-slate-200">{{ t('abbreviation') }}</label>
                    <TextInput v-model="form.abbreviation" class="mt-1 w-full" placeholder="GK" required />
                </div>
                <div class="flex-1">
                ...
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <tr v-for="pos in positions" :key="pos.id">
                            <td class="px-4 py-3">
                                <TextInput v-if="editingId === pos.id" v-model="editForm.abbreviation" class="w-16" />
                                <span v-else class="rounded bg-slate-100 dark:bg-slate-800 px-2 py-0.5 font-mono text-xs font-bold text-slate-700 dark:text-slate-200">{{ pos.abbreviation }}</span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-end">
                                ...
                                <div v-else class="flex justify-end gap-2">
                                    <button @click="startEdit(pos)" class="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">{{ t('edit') }}</button>
                                    <button @click="deleteId = pos.id" class="text-sm text-rose-500 hover:text-rose-700">{{ t('delete') }}</button>
                                </div>
                            </td>
                        </tr>
